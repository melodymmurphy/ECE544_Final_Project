...
uint32_t PMODENC544_getBtnSwReg(void)
{
    ...
    if (isInitialized) {
        btnsw = PMODENC544_mReadReg(baseAddress, PMODENC544_PMODENC_BTNSWT_REG_OFFSET);
    }
    else {
        btnsw = 0xDEADBEEF;
    }
...
uint32_t PMODENC544_clearRotaryCount(void)
{
    ...
    if (isInitialized) {
        ...
        count = 0x00000001;
        PMODENC544_mWriteReg(baseAddress, PMODENC544_PMODENC_CLR_ROTARY_COUNT_REG_OFFSET, count);
        PMODENC544_mWriteReg(baseAddress, PMODENC544_PMODENC_CLR_ROTARY_COUNT_REG_OFFSET, 0x0);
