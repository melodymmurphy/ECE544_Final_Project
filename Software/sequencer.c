...
{
	note->velocity	= AMP_MAX;		// intialize velocity value to half of maxmium
	note->pitch = pitch;			// initialize to desired pitch
	note->duty_cycle = DC_MAX / 2;	// intialize duty cycle value to half of maxmium
	note->note_on = true;
...
{
	initializeNote(&(sequencer->note[0]),  50);  	// Initialize sequence to D dorian scale
...
// update AXI timer based on current tempo, note subdivision, and swing
uint32_t tempo_timer(sequencer_t* sequencer, uint8_t tempo)
{
	uint32_t timer_count;
	if ((sequencer->step % 2) == 0)	// if even step
	{
		timer_count = (TIMER_CLOCK / tempo) * (SWING_MAX + sequencer->swing);	// set timer to tempo and swing by selected percentage
	}
	else	// if odd step
	{
	...
	timer_count /= SWING_MAX;	// apply swing as percetage
	timer_count = (timer_count >> (sequencer->subdiv)) + 1;		// add one to avoid 0 count timer cases
	timer_count *= 60;	// convert from BPS to BPM
	return timer_count;
}
