...
{
	uint32_t reg = MIDI_processor_readInterrupts();	// read, clear and disable interrupts
	MIDI_processor_clearDisableInterrupts();

	if (reg & NOTE_MASK)			// note on
	{
		xSemaphoreGiveFromISR(note_sem, NULL);	// notify system that note state has changed
	}
	else if (reg & MODULATION_MASK)	// modulation
	{
		xSemaphoreGiveFromISR(mod_sem, NULL);	// notify system that modulation state has changed
	}

	MIDI_processor_enableInterrupts();	// re-enable interrupts
}

void GPIO_Handler(void)
{
	uint8_t btns = getButtons();	// get current button state from GPIO module
	if ((btns & BTNU) || (btns & BTND))		// if up or down buttons pressed
	{
		xSemaphoreGiveFromISR(display_sem, NULL);			// notify display that GPIO state has changed
	}
	else if ((btns & BTNL) || (btns & BTNR) || (btns & BTNC))	// if let, right, or center buttons pressed
	{
